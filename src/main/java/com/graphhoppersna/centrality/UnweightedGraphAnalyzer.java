...
//                    + pathLengths.getCount());
     ...
     * in the network (assuming all edges have weight 1) and stores them in a
     * {@link PathLengthData} object.
     *
     * This method uses a breadth-first traversal (BFS) through the network,
     * starting from the specified node, in order to find all reachable nodes
     * and accumulate their distances. This only works if all edges are
     * considered to have the same weight (1.0).
     *
     ...
     */
    private PathLengthData computeShortestPathsData(int startNode) {
